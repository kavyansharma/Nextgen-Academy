...
export default function About() {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 py-20 overflow-hidden">
      ...
      {/* Background glow */}
      <div className="absolute top-1/3 right-0 w-[400px] h-[400px] bg-blue-500/5 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative space-y-20">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          ...
          <h1 className="text-4xl font-extrabold text-slate-900 sm:text-5xl leading-tight">
            Engineering Success, <span className="text-blue-600">Developing Leaders</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 space-y-6">
            <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
              <Building2 className="w-6 h-6 text-blue-650" />
          ...
          <div className="lg:col-span-5">
            <div className="p-8 rounded-3xl bg-white border border-slate-200 shadow-xl space-y-6">
              <h3 className="text-slate-900 font-bold text-lg">Our Unique Advantage</h3>
              <ul className="space-y-4">
                {points.map((pt, idx) => {
                  const Icon = pt.icon;
                  return (
                    <li key={idx} className="flex gap-4">
                      <div className="p-2 rounded-lg bg-slate-50 border border-slate-100 text-blue-600 h-fit">
                        <Icon className="w-5 h-5" />
                      </div>
                      <div>
                        <h4 className="text-sm font-semibold text-slate-800">{pt.title}</h4>
                        <p className="text-xs text-slate-550 mt-1 leading-relaxed">{pt.description}</p>
                      </div>
                    </li>
                  );
                })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {pillars.map((pillar, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className="p-8 rounded-3xl bg-white border border-slate-200 shadow-sm hover:border-blue-600/30 transition-all duration-300 flex items-start gap-5 group"
              >
                <div className={`p-4 rounded-2xl bg-slate-50 border border-slate-100 group-hover:scale-105 transition-transform duration-300 ${pillar.color}`}>
