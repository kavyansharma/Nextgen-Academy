...
export default function Values() {
  ...
  return (
    <div className="relative min-h-screen bg-brand-dark py-20 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/3 w-[500px] h-[500px] bg-brand-blue/5 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative space-y-20">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          ...
          <h1 className="text-4xl font-extrabold text-white sm:text-5xl leading-tight">
            Built on <span className="text-brand-orange">T.R.U.S.T.</span>
          </h1>
        ...
        <div className="relative flex justify-center py-6">
          <div className="flex gap-4 md:gap-8 text-5xl md:text-8xl font-black tracking-tighter opacity-20">
            {valuesList.map((val, idx) => (
              <span key={idx} className="hover:opacity-100 cursor-default hover:text-brand-orange transition-all duration-300 select-none">
                {val.letter}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {valuesList.map((value, idx) => {
            ...
            return (
              <div 
                key={idx}
                className="group relative p-8 rounded-3xl bg-brand-dark-light border border-slate-800 hover:border-slate-700 transition-all duration-300 flex flex-col justify-between"
              >
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-2xl text-white ${value.bgColor}`}>
                      {value.letter}
                    </div>
                    <div className={`p-2.5 rounded-lg bg-slate-900 border border-slate-850 ${value.color}`}>
                  ...
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-400 italic">
                ...
                <div className="pt-6 border-t border-slate-800/80 mt-6 flex items-center gap-2 text-xs font-semibold text-slate-400">
