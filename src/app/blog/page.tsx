    ...
    <div className="relative min-h-screen bg-slate-50 flex flex-col justify-between overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 relative z-10 w-full">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto mb-16">
          <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-blue-650 text-xs font-semibold uppercase tracking-wider">
            <BookOpen className="w-3.5 h-3.5" />
            NextGen Insights
          </span>
          <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight leading-none">
            Industrial Strategy & <span className="bg-gradient-to-r from-blue-600 to-indigo-650 bg-clip-text text-transparent">Operations Blog</span>
          </h1>
          <p className="text-sm sm:text-md text-slate-600 max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {BLOG_POSTS.map((post) => (
            <div
              key={post.id}
              className="p-6 rounded-3xl bg-white border border-slate-200 shadow-md flex flex-col justify-between hover:border-blue-600/30 transition-all duration-300 group"
            >
              <div className="space-y-4">
                {/* Meta details */}
                <div className="flex justify-between items-center gap-4">
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-slate-50 text-blue-650 border border-slate-100">
                    {post.category}
                  </span>
                ...
                <h2 className="text-xl font-bold text-slate-900 leading-tight group-hover:text-blue-600 transition-colors duration-200">
                  {post.title}
                </h2>
              ...
              {/* Author & Read CTA */}
              <div className="pt-6 border-t border-slate-100 mt-6 flex justify-between items-center text-[10px] text-slate-500">
