      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 rounded-lg bg-gradient-to-tr from-brand-orange to-brand-blue flex items-center justify-center font-bold text-lg text-white shadow-sm">
                N
              </div>
            ...
            <div className="flex items-center gap-4 pt-2">
              <a href="#" className="w-9 h-9 rounded-lg bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-650 hover:text-brand-orange hover:border-brand-orange transition-all duration-300">
          ...
          {/* Services */}
          <div>
            <h3 className="text-slate-900 font-bold text-sm tracking-wider uppercase mb-6">Our Services</h3>
            <ul className="space-y-3">
              {services.map((item) => (
                <li key={item.name}>
                  <Link href={item.href} className="text-sm text-slate-600 hover:text-brand-orange transition-colors duration-200 flex items-center gap-1.5 group">
          ...
          {/* Industries */}
          <div>
            <h3 className="text-slate-900 font-bold text-sm tracking-wider uppercase mb-6">Industries Covered</h3>
            <ul className="space-y-3">
              {industries.map((item) => (
                <li key={item.name}>
                  <Link href={item.href} className="text-sm text-slate-600 hover:text-brand-orange transition-colors duration-200 flex items-center gap-1.5 group">
          ...
          {/* Contact Details */}
          <div className="space-y-6">
            <h3 className="text-slate-900 font-bold text-sm tracking-wider uppercase">Get in Touch</h3>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <a href="tel:+919971524733" className="text-sm text-slate-600 hover:text-brand-orange transition-colors duration-200">
                  +91 99715 24733
                </a>
              </div>
              <div className="flex items-center gap-3">
                <Mail className="w-5 h-5 text-brand-orange flex-shrink-0" />
                <a href="mailto:info@nextgen-consulting.com" className="text-sm text-slate-600 hover:text-brand-orange transition-colors duration-200">
        ...
        <div className="border-t border-slate-200 mt-12 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-6">
            {quickLinks.map((item) => (
              <Link key={item.name} href={item.href} className="text-xs text-slate-500 hover:text-brand-orange transition-colors duration-200">
                {item.name}
              </Link>
            ))}
          </div>
